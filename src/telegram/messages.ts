...
export async function getAllMessages(
  { client, user: { accessHash, peerId: userId, firstName }, offsetId, chatAreaWidth }: {
    client: TelegramClient, user: ChatUser, offsetId?: number,
  ...
  const orgnizedMessages = (
    await Promise.all(
      messages.reverse()?.map(async (message): Promise<FormattedMessage> => {
        ...
        const imageString = await (buffer ? terminalImage.buffer(new Uint8Array(buffer), {
          width
        }) : null)
