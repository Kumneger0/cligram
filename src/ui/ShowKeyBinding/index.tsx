...
function ShowKeyBinding({ type }: ShowKeyBindingProps) {
    ...
    const keyBindingToShow = Object.entries(type !== 'general' ? { ...keyBindings.general, ...keyBindings[type] } : keyBindings[type]).filter(([_key, value]) => value.mode === all || value.mode === currentChatType)
    return (
        <Box>
            <Text>
