...
export const conversationStore = create<{
	...
	return {
		...
		setConversation: (conversation) => {
			return set(() => ({ conversation: conversation }));
		},
		updateConversations: (conversation) => {
			return set((state) => ({ conversation: [...state.conversation, ...conversation] }));
		}
	};
});
...
export const useTGCliStore = create<TGCliStore>((set, get) => {
	return {
		...
		setCurrentlyFocused: (currentlyFocused) => {
			return set((state) => {
...
export const useForwardMessageStore = create<{
	...
	return {
		...
		setForwardMessageOptions: (forwardMessageOptions) => {
			return set((state) => {
